...
export default function App() {
  ...
  // Tab button — whitespace-nowrap + shrink-0 enables horizontal scroll on mobile
  const TabBtn = ({ id, label, icon: Icon }) => {
    ...
    return (
      <button
        ...
        className={
          'pb-3 px-2 font-medium text-sm transition-colors border-b-2 flex items-center gap-1.5 whitespace-nowrap shrink-0 ' +
          (isActive ? 'border-amber-500 text-amber-400' : 'border-transparent text-slate-400 hover:text-slate-200')
        }
      >
        {Icon && <Icon size={14} />}
        {label}
  ...
  // Shared info card for performance + entropy tabs
  const InfoCard = ({ icon: Icon, title, children }) => (
    <div className="bg-slate-900 p-5 rounded-xl border border-slate-800">
      <h4 className="font-bold text-amber-400 mb-3 flex items-center gap-2 text-sm md:text-base">
        <Icon size={16} /> {title}
      </h4>
      {children}
    </div>
  ...
  const renderLineChart = () => {
    ...
    return (
      <div className="relative w-full bg-slate-900 rounded-xl p-4 border border-slate-700 shadow-lg">
        <h3 className="text-base md:text-lg font-bold text-slate-100 mb-4 flex items-center gap-2">
          <Activity size={20} className="text-blue-400 shrink-0" />
          Round-by-Round Avalanche Progression
        </h3>
        <div className="overflow-x-auto">
          <svg viewBox={'0 0 ' + chartWidth + ' ' + chartHeight} className="w-full min-w-[560px] h-auto">
            ...
            <text x={chartWidth - padding + 10} y={targetY + 4} fill="#10b981" fontSize="12" fontWeight="bold">50%</text>
        ...
        <div className="flex flex-wrap justify-center gap-3 md:gap-6 mt-4">
          <div className="flex items-center gap-2"><div className="w-3 h-3 md:w-4 md:h-4 bg-blue-500 rounded-sm"></div><span className="text-slate-300 text-xs md:text-sm font-medium">Original DES</span></div>
          <div className="flex items-center gap-2"><div className="w-3 h-3 md:w-4 md:h-4 bg-amber-500 rounded-sm"></div><span className="text-slate-300 text-xs md:text-sm font-medium">Modified DES (DSR)</span></div>
          <div className="flex items-center gap-2"><div className="w-3 h-0.5 md:w-4 bg-emerald-500 border-t-2 border-dashed border-emerald-500"></div><span className="text-slate-300 text-xs md:text-sm font-medium">Ideal (32 bits)</span></div>
        </div>
      </div>
    );
  };
  ...
  const renderBitMatrix = (matrix, title, percentage, colorClass) => (
    <div className="bg-slate-900 border border-slate-700 rounded-xl p-4 md:p-6 shadow-lg flex-1">
      <div className="flex justify-between items-center mb-4 md:mb-6 gap-2">
        <h3 className="text-sm md:text-lg font-bold text-slate-100 leading-tight">{title}</h3>
        <div className={'px-2 md:px-3 py-1 rounded-full text-xs md:text-sm font-bold shrink-0 ' + colorClass + ' bg-opacity-10 border'}>{percentage}% Flipped</div>
      </div>
      <div className="grid grid-cols-8 gap-1 md:gap-2">
        {matrix.map((isFlipped, idx) => (
          <div
            key={idx}
            className={'h-7 md:h-8 rounded flex items-center justify-center text-xs font-mono transition-all duration-300 ' +
              (isFlipped
    ...
    return (
      <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
          <InfoCard icon={FastForward} title="What it is">
          ...
          <InfoCard icon={Cpu} title="How to test">
            <ol className="text-sm text-slate-400 list-decimal list-inside space-y-1.5 leading-relaxed">
          ...
          <InfoCard icon={Activity} title="Why it's good">
        ...
        {/* Benchmark bars */}
        <div className="bg-slate-950 p-5 md:p-8 rounded-xl border border-slate-800">
          <h4 className="font-bold text-slate-200 mb-6 md:mb-8 flex items-center gap-2 text-sm md:text-base">
            <Database className="text-slate-400 shrink-0" size={18} /> Benchmark Results — Encryption Time (seconds)
          </h4>
              ...
              <div key={idx}>
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-2 md:gap-4">
                    <div className="w-20 md:w-28 text-xs md:text-sm text-slate-400 text-right font-medium shrink-0">Original DES</div>
                    <div className="flex-1 h-3 bg-slate-800 rounded-full overflow-hidden">
                      <div className="bg-blue-600 h-full rounded-full" style={{ width: (item.originalTime / maxTime * 100) + '%' }}></div>
                    </div>
                    <div className="w-12 md:w-16 text-xs md:text-sm text-slate-400 font-mono text-right shrink-0">{item.originalTime.toFixed(3)}s</div>
                  </div>
                  <div className="flex items-center gap-2 md:gap-4">
                    <div className="w-20 md:w-28 text-xs md:text-sm text-amber-400 text-right font-medium shrink-0">Modified DES</div>
                    <div className="flex-1 h-3 bg-slate-800 rounded-full overflow-hidden">
                      <div className="bg-amber-500 h-full rounded-full shadow-[0_0_8px_rgba(245,158,11,0.4)]" style={{ width: (item.modifiedTime / maxTime * 100) + '%' }}></div>
                    </div>
                    <div className="w-12 md:w-16 text-xs md:text-sm text-amber-400 font-mono text-right font-medium shrink-0">{item.modifiedTime.toFixed(3)}s</div>
                  </div>
                </div>
              </div>
            ))}
          </div>
          <p className="text-xs text-slate-500 mt-6 md:mt-8 pt-5 border-t border-slate-800">
            * The DSR bitwise circular shift introduces virtually zero computational overhead (less than 2% across all file sizes).
          </p>
        </div>

        {/* Throughput — cards on mobile, table on desktop */}
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 md:p-6 shadow-lg">
          <h4 className="font-bold text-slate-200 mb-4 md:mb-6 flex items-center gap-2 text-sm md:text-base">
            <TrendingUp size={18} className="text-amber-400 shrink-0" /> Throughput Comparison (MB/s)
          </h4>
          {/* Mobile cards */}
            ...
            {PERFORMANCE_DATA.map((item, idx) => {
              ...
              return (
                <div key={idx} className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
                  <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">{item.size}</div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <div className="text-xs text-slate-500 mb-1">Original DES</div>
                      <div className="font-mono text-blue-300 font-semibold">{item.originalThroughput.toFixed(1)} MB/s</div>
                    </div>
                    <div>
                      <div className="text-xs text-slate-500 mb-1">Modified DES</div>
                      <div className="font-mono text-amber-300 font-semibold">{item.modifiedThroughput.toFixed(1)} MB/s</div>
                    </div>
                  </div>
                  <div className="mt-3 pt-2 border-t border-slate-700 text-xs text-slate-500">
                    Overhead: <span className="text-amber-400 font-mono font-semibold">+{overhead}%</span>
                  </div>
                </div>
              );
            })}
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 md:p-6">
          <h4 className="text-amber-400 font-bold mb-2 flex items-center gap-2 text-sm md:text-base">
            <CheckCircle2 size={18} className="shrink-0" /> Key Observation from Benchmark
          </h4>
          <p className="text-slate-300 text-sm leading-relaxed">
            Across all tested file sizes, the <strong>Modified DES (DSR)</strong> introduces less than <strong>2% additional processing time</strong> compared to Original DES. The DSR modification — a single left circular shift per round — is a single-clock CPU operation. This means our security improvement comes at virtually no performance cost, making DSR a practical and deployable enhancement.
          </p>
        </div>
      </div>
    );
  ...
  const renderEntropyTab = () => {
    ...
    const Gauge = ({ value, color, label }) => {
      ...
      return (
        <div className="flex flex-col items-center">
          <svg width="130" height="105" viewBox="0 0 160 120">
          ...
          <span className="text-xs md:text-sm font-semibold mt-1" style={{ color }}>{label}</span>
        </div>
      );
    };
    ...
    return (
      <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
          <InfoCard icon={BarChart2} title="What it is">
            <p className="text-sm text-slate-400 leading-relaxed">
          ...
          <InfoCard icon={Shuffle} title="How to test">
            <ol className="text-sm text-slate-400 list-decimal list-inside space-y-1.5 leading-relaxed">
          ...
          <InfoCard icon={Binary} title="Why it's good">
            <p className="text-sm text-slate-400 leading-relaxed">
              Produces a single objective number. If Modified DES scores <strong className="text-slate-300">7.9932</strong> vs Original's <strong className="text-slate-300">7.9808</strong>, you can objectively argue your modification improved unpredictability.
            </p>
          </InfoCard>
        </div>

        {/* Gauge section */}
        <div className="bg-slate-950 rounded-xl border border-slate-800 p-5 md:p-8">
          <h4 className="font-bold text-slate-200 mb-5 md:mb-6 flex items-center gap-2 text-sm md:text-base">
            <BarChart2 size={18} className="text-amber-400 shrink-0" /> Average Entropy Score (bits / byte)
          </h4>
          <div className="flex flex-row items-center justify-around gap-2 md:gap-8">
            <Gauge value={parseFloat(avgOriginal)} color="#3b82f6" label="Original DES" />
            <div className="flex flex-col items-center gap-1 md:gap-2">
              <div className="text-xs text-slate-500 uppercase tracking-widest text-center leading-tight">Difference</div>
              <div className="text-lg md:text-2xl font-extrabold text-amber-400 font-mono">
                +{(parseFloat(avgModified) - parseFloat(avgOriginal)).toFixed(4)}
              </div>
              <div className="text-xs text-slate-500 text-center leading-tight">bits / byte</div>
            </div>
        ...
        {/* Per-sample — cards on mobile, table on desktop */}
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 md:p-6 shadow-lg">
          <h4 className="font-bold text-slate-200 mb-4 md:mb-6 flex items-center gap-2 text-sm md:text-base">
            <Database size={18} className="text-amber-400 shrink-0" /> Per-Sample Entropy Results
          </h4>
          {/* Mobile cards */}
          <div className="md:hidden space-y-3">
            {[...ENTROPY_DATA, {
              sample: 'Average', plaintext: null,
              original: parseFloat(avgOriginal), modified: parseFloat(avgModified), isAvg: true,
            }].map((row, idx) => {
              const delta = (row.modified - row.original).toFixed(4);
              return (
                <div key={idx} className={`rounded-lg p-4 border ${row.isAvg ? 'bg-slate-800/60 border-slate-600' : 'bg-slate-800/30 border-slate-700'}`}>
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <div className={`text-xs font-bold ${row.isAvg ? 'text-slate-300' : 'text-slate-400'}`}>{row.sample}</div>
                      {row.plaintext && <div className="text-xs text-slate-500 font-mono mt-0.5">{row.plaintext}</div>}
                    </div>
                    <span className="text-amber-400 font-mono font-semibold text-sm">+{delta}</span>
                  </div>
                  <div className="grid grid-cols-2 gap-3 pt-2 border-t border-slate-700">
                    <div>
                      <div className="text-xs text-slate-500 mb-1">Original (H)</div>
                      <div className="font-mono text-blue-300 text-sm">{row.original.toFixed(4)}</div>
                    </div>
                    <div>
                      <div className="text-xs text-slate-500 mb-1">Modified (H)</div>
                      <div className="font-mono text-amber-300 text-sm">{row.modified.toFixed(4)}</div>
                    </div>
                  </div>
                </div>
              );
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 md:p-6">
          <h4 className="text-amber-400 font-bold mb-2 flex items-center gap-2 text-sm md:text-base">
            <CheckCircle2 size={18} className="shrink-0" /> Key Observation from Entropy Analysis
          </h4>
          <p className="text-slate-300 text-sm leading-relaxed">
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 p-4 md:p-8 font-sans selection:bg-amber-500/30">
      <div className="max-w-6xl mx-auto space-y-6 md:space-y-8">
        ...
        <header className="border-b border-slate-800 pb-5 md:pb-6">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-amber-500/10 border border-amber-500/20 text-amber-400 rounded-full text-xs md:text-sm font-medium mb-3 md:mb-4">
            <Shield size={14} />
            Case Study Analysis Dashboard
          </div>
          <h1 className="text-2xl md:text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-orange-500 tracking-tight">
            DES vs. Modified DES (DSR)
          </h1>
          <p className="text-slate-400 mt-2 text-sm md:text-lg max-w-3xl">
        ...
        {/* Stat cards */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 md:p-5 shadow-md flex flex-col justify-between">
            <div className="flex items-center gap-2 text-slate-400 mb-2">
              <Key size={16} />
              <span className="text-xs font-semibold uppercase tracking-wider">Test Condition</span>
            </div>
            <div className="text-xl md:text-2xl font-bold text-slate-100">1-Bit Plaintext Flip</div>
            <p className="text-xs text-slate-500 mt-1">Measuring exact bit variations in the 64-bit ciphertext output.</p>
          </div>
          <div className="bg-blue-900/20 border border-blue-500/30 rounded-xl p-4 md:p-5 shadow-md flex flex-col justify-between">
            <div className="flex items-center gap-2 text-blue-400 mb-2">
              <Zap size={16} />
              <span className="text-xs font-semibold uppercase tracking-wider">Original Diffusion</span>
            </div>
            <div className="text-2xl md:text-3xl font-extrabold text-blue-400">48.4%</div>
            <p className="text-xs text-blue-300/70 mt-1">Average bits flipped after 16 rounds (approx 31/64).</p>
          </div>
          <div className="bg-amber-900/20 border border-amber-500/30 rounded-xl p-4 md:p-5 shadow-md flex flex-col justify-between">
            <div className="flex items-center gap-2 text-amber-400 mb-2">
              <TrendingUp size={16} />
              <span className="text-xs font-semibold uppercase tracking-wider">Modified Diffusion</span>
            </div>
            <div className="text-2xl md:text-3xl font-extrabold text-amber-400">51.5%</div>
            <p className="text-xs text-amber-300/70 mt-1">Average bits flipped after 16 rounds (approx 33/64).</p>
          </div>
        </div>

        {/* Tab nav — scrollable on mobile */}
        <div className="overflow-x-auto -mx-4 px-4 md:mx-0 md:px-0 border-b border-slate-800">
          <div className="flex gap-3 md:gap-4 pb-px w-max md:w-auto md:flex-wrap">
            <TabBtn id="graphs"      label="Avalanche Graph Analysis" />
            <TabBtn id="matrix"      label="Visual Bit-Flip Matrix" />
            <TabBtn id="explanation" label="Why DSR Works" />
            <TabBtn id="performance" label="Execution Time" icon={Clock} />
            <TabBtn id="entropy"     label="Shannon Entropy" icon={BarChart2} />
          </div>
        </div>
        ...
        <div className="min-h-[400px]">
          ...
          {activeTab === 'graphs' && (
            <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
              {renderLineChart()}
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 md:p-6">
                <h4 className="text-amber-400 font-bold mb-2 flex items-center gap-2 text-sm md:text-base"><CheckCircle2 size={18} /> Key Observation from Graph</h4>
                <p className="text-slate-300 text-sm leading-relaxed">
          ...
          {activeTab === 'explanation' && (
            <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 md:p-6 shadow-md">
                  <div className="w-10 h-10 rounded-full bg-blue-500/20 flex items-center justify-center mb-4 text-blue-400">
                    <Info size={24} />
                  </div>
                  <h3 className="text-lg md:text-xl font-bold text-slate-100 mb-3">The Limitation of Original DES</h3>
                ...
                <div className="bg-gradient-to-br from-slate-900 to-amber-950/30 border border-amber-500/20 rounded-xl p-5 md:p-6 shadow-md relative overflow-hidden">
                  ...
                  <h3 className="text-lg md:text-xl font-bold text-amber-400 mb-3 relative z-10">Why Our Modified DSR is Better</h3>
